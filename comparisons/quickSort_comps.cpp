...
int partition(int* array, int low, int high, long & comparisons) {
    int pivot = array[low];
    int i = low - 1;
    int j = high + 1;
    comparisons++;
...
void quickSort(int* array, int p, int r, long & comparisons) {
    comparisons++;
    if(p < r) {
        int q = partition(array, p, r, comparisons);
        quickSort(array, p, q, comparisons);
        quickSort(array, (q + 1), r, comparisons);
    }
}
...
long clock(int* array, int size, long & comparisons) {
    ...
    quickSort(array, 0, (size - 1), comparisons);
...
int partitionBestCase(int* array, int low, int high, long & comparisons) {
...
void quickSortBestCase(int* array, int p, int r, long & comparisons) {
    comparisons++;
    if(p < r) {
        int q = partitionBestCase(array, p, r, comparisons);
        quickSortBestCase(array, p, q, comparisons);
        quickSortBestCase(array, (q + 1), r, comparisons);
    }
}
...
long clockBestCase(int* array, int size, long & comparisons) {
    auto start = high_resolution_clock::now();
    quickSortBestCase(array, 0, (size - 1), comparisons);
    auto stop = high_resolution_clock::now();
    auto duration = duration_cast<nanoseconds>(stop - start);
    long clockTime = duration.count();
...
void runSort(ifstream & txtFile, string fileName, int size) {
    ...
    for(int i = 0; i < 15; i++) {
        ...
        if (!txtFile.is_open()) {
            ...
            long currBest = clockBestCase(bestCase, size, comparesB);
            if(i >= 5) {
                sumBest += currBest;
            };
    ...
    cout << "Number of Comparisons: " << comparesB / 10 << endl;
    cout << endl;
    ...
    cout << "Number of Comparisons: " << comparesW / 10 << endl;
    cout << endl;
    ...
    cout << "Number of Comparisons: " << comparesA / 10 << endl;
    cout << endl << endl << endl;
